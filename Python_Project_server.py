...
def server_program():
    ...
    conn1.send("You are Player 1. Game starts!\n".encode())
...
def play_game(player_conn, opponent_conn, player_name):
    ...
    opponent_conn.send(f"{player_name} made their choice \n".encode())
    ...
    if player_choice == opponent_choice:
        ...
        outcome = f"{player_name} wins!\n"
    else:
        outcome = f"{player_name} loses!\n"
    
    # Send outcome to both players
    player_conn.send(outcome.encode())
    opponent_conn.send(outcome.encode())
